...
**Not included in this pass**: the Prometheus/Grafana/Alertmanager stack,
Splunk logging, and the SLO/burn-rate alerting that existed in the
Kubernetes deployment were all cluster-native (`kube-prometheus-stack`,
`ServiceMonitor`, `PrometheusRule`) and don't carry over to EC2 as-is.
Equivalent observability here would mean CloudWatch alarms/dashboards, or
standing up Prometheus+Grafana on their own EC2 instance(s) — worth doing
as a deliberate follow-up, not bundled into this infrastructure change.
...
data processing), the ALB (~$0.025/hr + LCU usage), and 2+ `t3.small`
instances. RDS `db.t3.micro` and CloudFront/S3 are cheap at this traffic
level. Run `terraform destroy` when you're done experimenting with it.
